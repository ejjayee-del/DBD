...
class GeneratedCertificate(models.Model):
    ...
    def __str__(self):
        return f"{self.template.template_name if self.template else 'Unknown Template'} - {self.recipient_name}"


class CertificateRequest(models.Model):
    """Store certificate requests from requesters"""
    STATUS_CHOICES = (
        ('pending', 'Pending'),
        ('approved', 'Approved'),
        ('rejected', 'Rejected'),
    ...
    # Request data - JSON format storing all filled fields
    ...
    # Link to generated certificate if approved
    ...
    def __str__(self):
        return f"{self.template.template_name} - {self.recipient_name} ({self.status})"
